...
import com.krishna.app.repository.PurchaseReportRepository;
import com.krishna.app.repository.ShoeRepository;
...
public class SportyShoeServiceImpl implements SportyShoeService {
	...
	public Shoe createShoe(Shoe shoe) throws BussinessException {
		...
		if (oldShoe != null)
			throw new BussinessException("Shoe already exists with id: " + id);
	...
	public Shoe getShoeById(int id) throws BussinessException {
		...
		try {
			if (id <= 0)
				throw new BussinessException("Shoe Id can not be negative or zero");
			shoe = shoesRepo.findById(id).get();
		} catch (NoSuchElementException e) {
			throw new BussinessException("Shoe not found with Id: " + id);
		}
		return shoe;
	}
	...
	public void deleteShoeById(int id) throws BussinessException {
		try {
			...
			throw new BussinessException("SHoe does not exist with id: " + id);
		}
	}
	...
	public PurchaseReport createPurchaseReport(PurchaseReport pr) throws BussinessException {
		...
		if (oldPr != null)
			throw new BussinessException("Purchase report already exists with id: " + id);
	...
	public PurchaseReport getPurchaseReportById(int id) throws BussinessException {
		...
		try {
			if (id <= 0)
				throw new BussinessException("Purchase Report Id can not be negative or zero");
			pr = prRepo.findById(id).get();
		} catch (NoSuchElementException e) {
			throw new BussinessException("Purchase Report not found with Id: " + id);
		}
		return pr;
	}
	...
	public void deletePurchaseReportById(int id) throws BussinessException {
		try {
			...
			throw new BussinessException("Puchase Report does not exist with Id: " + id);
		}
	}
